...
var questions = require('../helpers/question.js');
...
function minChoicesValidator(min, message) {
  if (!min) min = 2;
  var msg = message || 'You must enter at least ' + min + ' choices.';

  return function (form, field, callback) {
    var choices = questions.splitChoices(field.data);
    if (choices.length < min) return callback(msg);
    callback();
  };
}
...
module.exports = function(options) {
  ...
  formFields['choices'] = fields.string({
    required: true,
    errorAfterField: true,
    validators: [minChoicesValidator()],
